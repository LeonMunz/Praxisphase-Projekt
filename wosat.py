...
# Erläuterungen
# Die einzulesenden Daten müssen bei WoS als Full Record im Format Win UTF-8 heruntergeladen werden.


# Leere Variablen auf None setzen:
ea = None
ka = None
sdir = None
sdir_res = None
...
#-----------------------------------------------------------------------------------------------------------------------
    def close_window():
        if os.path.isfile('./Zusammengeführte_Files/collection.txt'):
            print('File exist')
...
#-----------------------------------------------------------------------------------------------------------------------
    ...
    def selectDir():
        global sdir
        global data_clean
        # Checken ob das "Collection-File" existiert, falls ja wird es gelöscht:
        if os.path.isfile('./Zusammengeführte_Files/collection.txt'):
            print('File exist')
            os.remove('./Zusammengeführte_Files/collection.txt')
...
#-----------------------------------------------Extraction Functions----------------------------------------------------
    ...
    def main_extraction():
        ...
        # If-Abfrage ob eine Suche nach Datum vorgenommen wurde.
        # Dazu wird die Variable "von" geprüft, enthält diese einen anderen Wert als "None" wird das Dataframe mit den
        # Einträgen welche die entsprechenden Datum Werte enthalten gefüllt.
        if von is None:
        ...
        # Zusammenführen der Dataframes der Erst- und Reprint Autoren
        complete_set = pd.concat([data_select_inst_ea, data_select_inst_rp])
        # Selektion der nötigen Attribute des Haupt-Dataframes, welcher auch das Ausgabe-File repräsentiert.
        complete_set = complete_set[['TI', 'Aut-Status', 'DT', 'PY', 'EA Institution', 'KoAut Institution',
                                     'RP Institution', 'Other Gold', 'DOAJ Gold', 'Green Published', 'Green Accepted',
                                     'Bronze', 'SC', 'SO', 'PU', 'FU', ]]


        # If-Abfrage zum Pfad für das Outfile-CSV.
        # Wenn die Variable "sdir-res" einen anderen Wert als None enthält, wird der entsprechende Pfad als Ablageort
        # verwendet
        # Andernfalls wird der Programmordner als Abgalgeort verwendet.
        if sdir_res is not None:
            complete_set.to_csv(sdir_res + '/main_ausgabe_file.csv')
            print('Outfile-CSV mit erweiterten Suchparametern erstellt. In: ' + str(sdir_res) + ' abgelegt.')

        else:
            complete_set.to_csv('main_ausgabe_file.csv')
            print('Outfile-CSV ohne erweiterte Suchparameter erstellt. Im Programmordner abgelegt.')


        # Die Funktion "count_oa_status" zählt die mit True angegebenen Einträge im Dataframe
        ...
        # Variablen um Anzahl der insgesamt gefunden Einträge im "Result-Window" anzuzeigen:
        ...
        # Zeigt an in wie vielen Fällen die Institution des Erstautors der des Reprintautors entspricht.
        dopplung = 0
        ...
        # Dataframe der Kooperationen des Erstautors
        koop_df = pd.DataFrame(Listen_Ranking(collect_koautli).items())
        koop_df.columns = ['institut', 'anzahl']
        koop_df = koop_df.sort_values('anzahl', ascending=False)
        # Variable für Anzeige in Pie:
        koop_df_pie = koop_df[:10]
...
#-------------------------------------------------CONTENT---------------------------------------------------------------
        ...
        'Um eine Analyse zu ermöglichen, müssen die von Ihnen heruntergeladenen Files im TSV-Format (Win UTF-8)\n'
        'vorliegen. Die einzelnen Files können Sie in Sets von jeweils 500 Einträgen bei WOS herunterladen.\n'
        'Eine detaillierte Anleitung finden Sie unter dem Menuepunkt Anleitung in der Kopfnavigationsleiste.\n'
        'www.webofknowledge.com\n'
                        )

        instruction_lbl = Label(base_Frame_center, fg='white smoke', bg='grey16', justify=LEFT, text=
        ' \n'
        'Bitte wählen Sie das Verzeichnis in dem sich die TSV-Files befinden: \n'
        ' '
                                )
        ...
        '\n'
        '\n'
        'Bitte wählen Sie das Verzeichnis in dem die Ergebnisdateien abgelegt werden sollen:'
        '\n'
        ...
        img = ImageTk.PhotoImage(Image.open("./img/TH_Koeln_Logo.png"))
...
# ------------------------------------------------CONTENT---------------------------------------------------------------
        ...
        '\nBitte geben Sie hier die zu suchenden Institutionen ein.\n'
        'Es müssen beide Felder ausgefüllt werden. Die suche ist nicht case sensitive.\n'
                        )
...
# ---------------------------------------------------FRAMES-------------------------------------------------------------
        ...
        center_content_Frame_top = Frame(center_space_Frame_center, width=700, height=190, bg='red')
        center_content_Frame_bottom = Frame(center_space_Frame_center, width=700, height=190, bg='blue')
        ...
        bottom_button_Frame_back.grid(row=0, column=1, sticky='n')
        bottom_button_Frame_forw.grid(row=0, column=2, sticky='e')
        ...
        desc_oa_auswahl_lbl = Label(center_content_Frame_bottom, bg='grey16', fg='white smoke', justify=LEFT,
                                    ...
                                    '\n' + 'Die meisten "Early Access" Publikationen besitzen keine Datierung daher besteht hier die möglichkeit ')


        # Ergebnis Label der Datumwerte
        # Wird ein Datum als Variable abgespeichert, dann erscheint hinter dem Eingabefeld das angegebene Datum.
        if von is not None:
            resul_date_von = Label(center_content_Frame_top, bg='grey19', fg='green2', text=von)
        else:
            resul_date_von = Label(center_content_Frame_top, bg='grey19', fg='green', text='     ')
        if bis is not None:
            resul_date_bis = Label(center_content_Frame_top, bg='grey19', fg='green2', text=bis)
            print('is da')
        else:
            resul_date_bis = Label(center_content_Frame_top, bg='grey19', fg='green', text='     ')
            print('is nich da')

        # Date search
        # Eingabefelder der Datumswerte
        ...
        oa_search_btn.grid(row=5, columnspan=5, pady=5, sticky='w')
            #Bottom BTNs:
        back_btn.grid(column=0, row=0, padx=0, pady=5, sticky='nwse')

        # CheckBTNs:
        # HIER MUSS NOCH IEN CHECKBTN FÜR AN UND ABWAHL VON EA PUBLIKATIONEN EINGEBAUT WERDEN
    ...
    def result_window():
        ...
        def ea_pie():
            ...
            canvas = FigureCanvasTkAgg(fig, center_Frame_right)
            canvas.get_tk_widget().grid(row=0, column=0)
        def kooperationen_ea():
            # Values for PieChart
            labels = koop_df_pie['institut']
            sizes = koop_df_pie['anzahl']
            ...
            plt.title('Top 10 Kooperationen', color='white')
        ...
        #Funktion um das DataFrame der Kooperationen zu exportieren
        def safe_koop():
            koop_safe_dir = filedialog.askdirectory()
            koop_df.to_csv(koop_safe_dir + '/anzahl_der_kooperationen.csv', index=FALSE)
            print('Kooperationen_csv_gespeichert.')
...
# ----------------------------------------------------------------------------------------------------------------------
        ...
        base_Frame_left = Frame(base_Frame, width=100, height=380, bg='grey16')
        base_Frame_rigth = Frame(base_Frame, width=100, height=380, bg='grey16')
        base_Frame_center = Frame(base_Frame, width=580, height=380, bg='grey16')
        ...
        center_Frame_left_bottom_two = Frame(center_Frame_left_bottom, width=135, height=200, bg='grey16')
        ...
        center_Frame_left_bottom_two.grid(row=0, column=1, sticky='w')
        ...
        center_Frame_left_bottom_two.grid_propagate(0)
        center_Frame_left.grid_propagate(0)
    ...
    # Export Part:
        # Export lbl
        export_desc_lbl = Label(center_Frame_left_bottom_two, bg='grey16', fg='white smoke', text='Daten Export:')

        # Export Position lbl:
        export_desc_lbl.grid(row=0, column=0, sticky='w')

        # Export BTNs:
        pie_export_btn = Button(center_Frame_left_bottom_two, text='Export Charts', bg='grey33')
        koop_export_btn = Button(center_Frame_left_bottom_two, text='Export Kooperationen', command=safe_koop)

        #Export BTNs Position:
        pie_export_btn.grid(row=1, column=0, sticky='w')
        koop_export_btn.grid(row=2, column=0, sticky='w')
    ...
    # Results LBL:
        ...
        ka_sum_lbl = Label(center_Frame_left, bg='grey16', fg='white smoke', text='Reprint Autoren der Institution:  ' + str(ka))
    ...
    #Chart BTNs:
        ...
        koop_pie_btn = Button(center_Frame_left_bottom_one, text=' Kooperationen ', command=kooperationen_ea)


    # Chart BTNs Position:
        pie_chart_mix_btn.grid(row=1, column=0, pady=2, sticky='w')
        pie_chart_ea_btn.grid(row=2, column=0, sticky='w')
        koop_pie_btn.grid(row=3, column=0, sticky='w')
    ...
    # Results VALUEs
        ...
        pie_desc_lbl = Label(center_Frame_left_bottom_one, bg='grey16', fg='white smoke', text='PieChart Auswahl:')
    ...
    def anleitung():
        ...
        back_btn = Button(bottom_button_Frame_back, text='   Zurück   ', command=mainWindowGrey)
        # BTNs in Bottom Frame:
        back_btn.grid(column=0, row=0, padx=0, pady=5, sticky='n')

        # Anleitung Content
        instruction_lbl = Label(base_Frame_center, fg='white smoke', bg='grey16',justify=LEFT,
                                text='Schrittweise Anleitung zur Verwendung des Programms: \n\n'
                                     '1. Download der Daten von der Datenbank Web of Science (www.webofknowledge.com)\n'
                                     '    Es muss zwingend ein Full Record im Format (Win UTF-8) extrahiert werden.\n'
                                     '2. Im ersten Fenster muss der Pfad gewählt werden in welchem sich die'
                                     ' heruntergeladenen Daten befinden,\n'
                                     '    die zweite Pfadoption legt den Speicherort der Ausgabedateien fest.'
                                     ' Diser ist optional\n'
                                     '3. Innerhalb des zweiten fensters können die zu suchenden Institutionen für den'
                                     ' Erst- als auch den Reprint Autor festgelegt werden.\n'
                                     '    Hier müssen beide Felder ausgefüllt werden.\n'
                                     '    zusätzlich befindet sich dort ein Button "Erweiterte Suche" hier kann die'
                                     ' Ergebnismenge auf das Erscheinungsjahr der jeweiligen\n    Veröffentlichungen'
                                     ' gefiltert werden\n'
                                     '4. Im letzten Fenster werden die wichtigsten Mengen in Form von Zahlen und Charts'
                                     'angezeigt.\n'
                                     '    Hier können die Charts sowie eine CSV-Datei exportiert werden.\n\n'
                                     'Für weitere Informationen steht das Repositorium auf GitHub zur Verfügung:\n'
                                     'https://github.com/LeonMunz/Praxisphase-Projekt')
...
# ----------------------------------------------------------------------------------------------------------------------
    ...
    window.iconbitmap('./img/wosat_icon.ico')

    # Menu
    menu = Menu(window)
    new_item = Menu(menu, tearoff=0)
    new_item.add_command(label='Anleitung', command=anleitung)
